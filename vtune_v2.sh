...
while true; do    
    echo "Choose 1 for single application profiling"
    echo "Choose 2 for dual application profiling"
    read prof
    if [[ $prof -eq 1 ]]; then
        while true; do
        ...
        while true; do
            echo "Enter path for executable #1"
            read -r executable
            if [[ -f "$executable" ]]; then
                echo "Successfully found executable."
                break
        ...
        while true; do
            echo "Enter path for executable #2"
            read -r executable2
